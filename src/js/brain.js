...
chrome.runtime.onMessage.addListener(
    function (request, sender, sendResponse) {
        ...
        if (request == "train") {
            ...
            var testDataClicked, testDataUnclicked;
            chrome.storage.local.get(['kbd-clicked', 'kbd-unclicked'], function (data) {
                console.log(data['kbd-clicked'][0]);
                testDataClicked = Array.from(data['kbd-clicked']);
                testDataUnclicked = Array.from(data['kbd-unclicked']);

                for (var i = 0; i < testDataClicked.length; i++) {
                    testData.push(testDataClicked[i]);
                    testLabels.push(1);
                }
                for (var i = 0; i < testDataUnclicked.length; i++) {
                    testData.push(testDataUnclicked[i]);
                    testLabels.push(0);
                }
                console.log("retraining!");
                forest.train(testData, testLabels);
